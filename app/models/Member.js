...
const memberSchema = new mongoose.Schema(
  {
    ...
    baptism: {
      ...
      baptismNotes: {
        type: String,
        trim: true,
        maxlength: [500, "Baptism notes cannot exceed 500 characters"],
      },
    },
    membership: {
      joinDate: {
...
memberSchema.pre("save", async function (next) {
  if (this.isNew && !this.memberId) {
    ...
    if (church) {
      const memberCount = await this.constructor.countDocuments({
...
memberSchema.methods.addAttendance = function (attendanceData) {
  const existingRecord = this.attendance.find(
    (record) =>
      record.serviceDate.toDateString() ===
        new Date(attendanceData.serviceDate).toDateString() &&
...
memberSchema.methods.addVolunteerTeam = function (teamId, role) {
  const existingTeam = this.volunteerTeams.find(
...
memberSchema.methods.removeVolunteerTeam = function (teamId) {
  const team = this.volunteerTeams.find(
    (team) => team.teamId.toString() === teamId.toString()
  );
...
memberSchema.statics.findByChurch = function (churchId, options = {}) {
  ...
  if (options.isActive !== undefined)
    query["membership.isActive"] = options.isActive;
  if (options.isFirstTimer !== undefined)
    query["firstTimer.isFirstTimer"] = options.isFirstTimer;
...
memberSchema.statics.getAttendanceStats = async function (
  churchId,
  startDate,
  endDate
) {
