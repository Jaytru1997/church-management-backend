...
const createDonation = async (req, res) => {
  try {
    const { churchId, donorId, amount, currency, category, description, paymentMethod, donorInfo } = req.body;
...
const initializeOnlineDonation = async (req, res) => {
  try {
    const { churchId, amount, currency, category, description, donorInfo, campaignId } = req.body;
    ...
    const paymentData = {
      ...
      contractCode: church.financial?.monnifyContractCode || process.env.MONNIFY_CONTRACT_CODE,
    ...
    if (paymentResponse.status === 'SUCCESS') {
      ...
      donation.paymentDetails.checkoutUrl = paymentResponse.responseBody.checkoutUrl;
      await donation.save();
...
const handlePaymentCallback = async (req, res) => {
  try {
    const { paymentReference, transactionReference, paidAmount, transactionHash, transactionStatus, paymentMethod, paidOn, transactionType } = req.body;
...
const getAllDonations = async (req, res) => {
  try {
    ...
    const { churchId, status, category, paymentMethod, startDate, endDate, search } = req.query;
    const filter = {};
...
const getDonationStats = async (req, res) => {
  try {
    ...
    const stats = await Donation.getDonationStats(filter.churchId, filter.createdAt?.$gte, filter.createdAt?.$lte);
